...
interface Props  {
 ...
 const ReminderEditForm:React.FC<Props>=({data})=> {
    ...
    const form = useForm<z.infer<typeof reminderFormSchema>>({
        ...
        defaultValues: {
            ...
            sendTime: data.sendTime ? new Date(data.sendTime) : new Date(),
    ...
    async function onSubmit(values: z.infer<typeof reminderFormSchema>) {
        form.reset()
        console.log(values)
        const url = `https://crm-backend-production-e80f.up.railway.app/api/reminder/${data._id}`
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <Button type="submit">Save<IoIosTimer className="w-4 h-4 ml-2" /></Button>
